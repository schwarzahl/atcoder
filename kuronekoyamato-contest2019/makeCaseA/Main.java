...
public class Main {
	...
	private void solve() {
		...
		/*
		Road[] roads = new Road[300];
		for (int i = 0; i < 300; i++) {
			roads[i] = new Road();
			roads[i].id = String.format("R%03d", i);
			roads[i].l = 1 + rand.nextInt(10000);
			roads[i].flug = rand.nextInt(2);
		}
		Map<String, Set<String>> crossId2RoadId = new HashMap<>();
		Cross[] crosses = new Cross[1000];
		//for ()
		int N = sc.nextInt();
