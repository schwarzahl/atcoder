...
public class Main {
	...
	private void solve() {
		...
		for (int i = 0; i < M; i++) {
			output[i % N].append(s[i]);
		}
		for (int i = 0; i < N; i++) {
			//output[i].setLength(N);
		}
		...
		int count = 0;
		while (System.currentTimeMillis() - time < 2500L) {
			int[] offset = new int[N];
			for (int i = 0; i < N; i++) {
				offset[i] = random.nextInt(N);
			}
			int score = score(output, start, N, offset);
			if (max < score) {
				max = score;
			}
			count++;
	...
	private int score(StringBuilder[] output, Node start, int N, int[] offset) {
		int point = 0;
		for (int row = 0; row < N; row++) {
			List<Node> list = new ArrayList<>();
			for (int col = 0; col < N + 8; col++) {
				List<Node> nextList = new ArrayList<>();
				char c = output[row].toString().charAt((col + offset[row]) % N);
				for (Node node : list) {
					if (node.next[c - 'A'] != null) {
						nextList.add(node.next[c - 'A']);
					}
				}
				list = new ArrayList<>();
				if (col < N) {
					Node next = start.next[c - 'A'];
					if (next != null) {
						list.add(next);
					}
				}
		...
		for (int col = 0; col < N; col++) {
			...
			for (int row = 0; row < N + 8; row++) {
				List<Node> nextList = new ArrayList<>();
				char c = output[row % N].toString().charAt((col + offset[row % N]) % N);
				for (Node node : list) {
					if (node.next[c - 'A'] != null) {
						nextList.add(node.next[c - 'A']);
					}
