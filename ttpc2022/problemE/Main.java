...
public class Main {
	...
	private void solve() {
		...
		int Q = sc.nextInt();
		//sc.nextLine();
		int[][] A_map = createMap(A);
		int[][] B_map = createMap(new StringBuilder().append(B).reverse().toString());
		StringBuilder output = new StringBuilder();
		for (int testCase = 0; testCase < Q; testCase++) {
			String S = sc.nextLine();
			String revS = new StringBuilder().append(S).reverse().toString();
			int ans = S.length();
			int[] A_check = check(S, A_map);
			int[] B_check = check(revS, B_map);
			for (int B_start_index = 1; B_start_index < S.length(); B_start_index++) {
				if (A_check[B_start_index - 1] != -1 && B_check[S.length() - B_start_index - 1] != -1) {
					ans = Math.min(ans, Math.abs(B_start_index * 2 - S.length()));
				}
			}
			output.append(ans == S.length() ? -1 : ans).append('\n');
		}
	...
	private int[] check(String S, int[][] map) {
		int[] arr = new int[S.length()];
		Arrays.fill(arr, -1);
		int p = 0;
		for (int S_index = 0; S_index < S.length() && p < map.length; S_index++) {
			int i = c2i(S.charAt(S_index));
			p = map[p][i];
			arr[S_index] = p;
			if (p == -1) {
				break;
	...
	private int[][] createMap(String str) {
		int[][] map = new int[str.length()][53];
		for (int i = 0; i < 53; i++) {
			char asc = i2c(i);
			int next = -1;
			for (int str_index = str.length() - 1; str_index >= 0; str_index--) {
				if (str.charAt(str_index) == asc) {
					next = str_index;
				}
				map[str_index][i] = next;
			}
		}
		return map;
	}
