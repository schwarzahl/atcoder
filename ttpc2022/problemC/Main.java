...
public class Main {
	...
	private void solve() {
		...
		Arrays.sort(E);
		System.out.println(lowCount(A, 2));

		long ans = 0L;
		for (int i = 0; i < N; i++) {
			ans = (ans + A[i] * lowCount(B, A[i]) * lowCount(C, A[i]) ) % 998244353L;
			ans = (ans + A[i] * lowCount(B, A[i]) * lowCount(D, A[i]) ) % 998244353L;
			ans = (ans + A[i] * lowCount(B, A[i]) * lowCount(E, A[i]) ) % 998244353L;
			ans = (ans + A[i] * lowCount(C, A[i]) * lowCount(D, A[i]) ) % 998244353L;
			ans = (ans + A[i] * lowCount(C, A[i]) * lowCount(E, A[i]) ) % 998244353L;
			ans = (ans + A[i] * lowCount(D, A[i]) * lowCount(E, A[i]) ) % 998244353L;
		}
	}

	private int lowCount(int[] arr, int border) {
		int low = 0;
