...
public class Main {
	...
	private void solve() {
		...
		Map<Integer, Set<Integer>> childlens = new HashMap<>();
		Vertex[] vertexes = new Vertex[N];
		dfs(map, childlens, vertexes, 1, 0, 1);
		...
		for (Vertex v : vertexes) {
			int id = v.id;
			int odd = !childlens.containsKey(id) ? 0 : -100000000;
			int even = 0;
			if (childlens.containsKey(id)) {
				for (int child : childlens.get(id)) {
					int nextOdd = Math.max(odd + dp[child][0], even + dp[child][1]);
					even = Math.max(even + dp[child][0], odd + dp[child][1]);
					odd = nextOdd;
				}
			}
			dp[id][0] = A[id] + even;
	...
	private void dfs(Map<Integer, Set<Integer>> map, Map<Integer, Set<Integer>> childlens, Vertex[] vertexes, int current, int prev, int depth) {
		vertexes[current - 1] = new Vertex(current, depth);
		for (int next : map.get(current)) {
			if (next == prev) continue;
			childlens.putIfAbsent(current, new HashSet<>());
			childlens.get(current).add(next);
			dfs(map, childlens, vertexes, next, current, depth + 1);
		}
	}
